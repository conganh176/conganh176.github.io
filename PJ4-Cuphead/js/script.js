...
var cupheadHeight = 155;
var cupheadWidth = 98;
var cupheadX = 0;
var cupheadY = canvas.height-cupheadHeight;
...
var cupheadTotalFrames = 8;
var cupheadCurrentFrame = 0;
var cupheadShift = 0;

function updateCupheadFrame() {
  cupheadCurrentFrame = ++ cupheadCurrentFrame % cupheadTotalFrames;
  cupheadShift = cupheadCurrentFrame * 98;
}

function drawCuphead() {
    updateCupheadFrame();
    
    if(rightPressed && cupheadX < canvas.width-cupheadWidth) {
        cupheadX += 5;
        // ctx.drawImage(paddleright, paddleX, paddleY);
    }
    else if(leftPressed && cupheadX > 0) {
        cupheadX -= 5;
        // ctx.drawImage(paddleleft, paddleX, paddleY);
    }
    if (upPressed && cupheadY > 0) {
        cupheadY -= 5;
    } 
    else if (downPressed && cupheadY < canvas.height-cupheadHeight) {
        cupheadY += 5;
    } 
    // ctx.rect(paddleX, paddleY, paddleWidth, paddleHeight);
    // ctx.fillStyle = "white";
    ctx.drawImage(cuphead, cupheadShift, 0, 98, 155, cupheadX, cupheadY, 98, 155);
}
